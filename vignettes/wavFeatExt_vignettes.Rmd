...
## Getting Started
Once the package is available on Bioconductor, it can be installed in the usual way:
...
```{r}
install.packages("C:/Users/maharaniau/OneDrive - University of Leeds/Research/R Programming/Packages/wavFeatExt5/wavFeatExt/wavFeatExt_1.1.4.tar.gz",type="source", repos = NULL)
library(wavFeatExt)
```
...
pca_feat <- get.pca(sim_dat, npc = 10)
ica_feat <- get.ica(sim_dat, ncomp = 10)

# Classification using PCA features

res_pca <- classif.pcaica(
  data = pca_feat,
  y    = y,
  method = "lasso",
  k = 5
...
# Classification using ICA features

res_ica <- classif.pcaica(
  data = ica_feat,
  y    = y,
  method = "lasso",
  k = 5
...
```{r}
## Wavelet features (detail + scaling)

det_coef <- wavFeatExt(sim_dat, type = "detail")
sca_coef <- wavFeatExt(sim_dat, type = "scaling")

set.seed(1)
res_wav <- classif.wavFeatExt(
  data  = sim_dat,
  y     = y,
  det   = det_coef,
  sca   = sca_coef,
  method = "lasso",
  k      = 5,
  ite    = 2    # number of CV repetitions
)


## Misclassification error per scale (and segmented baseline)

plot(res_wav, type = "CE", ylab = "Misclassification error")

## AUC per scale (and segmented baseline)

plot(res_wav, type = "AUC", ylab = "Area under ROC curve")
